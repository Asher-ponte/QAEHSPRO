...
export async function GET(
    ...
    try {
        const user = await db.get('SELECT id, username, department, position, role FROM users WHERE id = ?', userId);
        if (!user) {
...
export async function PUT(
    ...
    try {
        ...
        await db.run(
            ...
            [username, department, position, role, userId]
        );
        
        const updatedUser = await db.get('SELECT id, username, department, position, role FROM users WHERE id = ?', userId);
