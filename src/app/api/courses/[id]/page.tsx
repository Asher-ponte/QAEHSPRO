...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    const [passedAssessmentRows] = await db.query<RowDataPacket[]>('SELECT id FROM final_assessment_attempts WHERE user_id = ? AND course_id = ? AND passed = 1', [userId, courseId]);
    const isCourseCompleted = passedAssessmentRows.length > 0;
