...
async function initializeDb() {
    ...
    // Drop course-related tables to ensure schema is always fresh in development.
    // In a production app, you would use a migration tool.
    await dbInstance.exec(`DROP TABLE IF EXISTS user_progress;`);
    await dbInstance.exec(`DROP TABLE IF EXISTS lessons;`);
    await dbInstance.exec(`DROP TABLE IF EXISTS modules;`);
    await dbInstance.exec(`DROP TABLE IF EXISTS courses;`);
    ...
    // Since tables are dropped, we can re-seed them every time.
